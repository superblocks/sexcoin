...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Sexcoin server has now stopped running!";
}
...
// Sexcoin: Return average network hashes per second based on last number of blocks.
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current Sexcoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Sexcoin address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid Sexcoin address:")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: Sexcoin address and we have full public key:
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Sexcoin server stopping, restart to run with encrypted wallet";
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
        result.push_back(Pair("algorithm", "scrypt:1024,1,1"));  // Sexcoin: specify that we should use the scrypt algorithm
        return result;
    }
    else
    {
...
//
// JSON-RPC protocol.  Sexcoin speaks version 1.0 for maximum compatibility,
